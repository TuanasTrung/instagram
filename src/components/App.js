...
function App() {
  ...
  const [linkAva, setLinkAva] = useState('https://afamilycdn.com/zoom/700_438/150157425591193600/2022/12/16/avatar1671163429004-1671163430431938690122-0-0-314-600-crop-16711635055311927524321.jpg');
            ...
            <div className="ig-top-center">
              ...
              <div className="infor">
                <div className="line-1">
                  <span className="account">tuanastrung</span>
          ...
          <div className="ig-right-bot">
            <div className='ig-bot-center'>
              <div className='menu-feed'>
                <div className='post-feed' onClick={() => setNavbar('POSTS')} style={{ color: navbar === 'POSTS' ? 'black' : 'rgb(153, 153, 153)', borderTop: navbar === 'POSTS' ? '1px solid black' : 'none' }}>
                  <img src={postFeed} style={{ opacity: navbar === 'POSTS' ? '100%' : '60%' }}></img>
                  <span className='posts'>POSTS</span>
                </div>
                <div className='reels-feed' onClick={() => setNavbar('REELS')} style={{ color: navbar === 'REELS' ? 'black' : 'rgb(153, 153, 153)', borderTop: navbar === 'REELS' ? '1px solid black' : 'none' }}>
                  <img src={reelsFeed} style={{ opacity: navbar === 'REELS' ? '100%' : '60%' }}></img>
                  <span className='reelss'>REELS</span>
                </div>
                <div className='saved-feed' onClick={() => setNavbar('SAVED')} style={{ color: navbar === 'SAVED' ? 'black' : 'rgb(153, 153, 153)', borderTop: navbar === 'SAVED' ? '1px solid black' : 'none' }}>
                  <img src={savedFeed} style={{ opacity: navbar === 'SAVED' ? '100%' : '60%' }}></img>
                  <span className='saved'>SAVED</span>
                </div>
                <div className='tagged-feed' onClick={() => setNavbar('TAGGED')} style={{ color: navbar === 'TAGGED' ? 'black' : 'rgb(153, 153, 153)', borderTop: navbar === 'TAGGED' ? '1px solid black' : 'none' }}>
                  <img src={taggedFeed} style={{ opacity: navbar === 'TAGGED' ? '100%' : '60%' }}></img>
                  <span className='tagged'>TAGGED</span>
                </div>
              </div>
              {navbar === 'POSTS' && <Feed />}
              {navbar === 'REELS' && <ReelsComponent />}
              {navbar === 'SAVED' && <SavedComponent />}
              {navbar === 'TAGGED' && <TaggedComponent />}
            </div>
          </div>
        </div>
      </div>
